# Source Of Truth Files

## Repo law and planning law

- AGENT.md
- README.md
- project.yml
- agent/progress.yaml
- agent/design/requirements.md
- agent/design/architecture.md
- agent/design/verification-strategy.md
- agent/design/repo-rules.md
- agent/design/source-of-truth-files.md
- agent/design/llm-terminal-execution-rules.md

## Milestone and task execution law

- agent/milestones/milestone-1-bootstrap.md
- agent/milestones/milestone-2-app-shell.md
- agent/milestones/milestone-3-live-connection.md
- agent/milestones/milestone-4-gift-catalog.md
- agent/tasks/milestone-1-bootstrap/task-1-create-repo-operating-system.md
- agent/tasks/milestone-2-app-shell/task-1-wire-minimal-app-shell.md
- agent/tasks/milestone-3-live-connection/task-1-replace-mock-connect-with-package-integration.md
- agent/tasks/milestone-4-gift-catalog/task-1-build-gift-and-interaction-list-surface.md
- agent/tasks/milestone-4-splash-screen/task-1-design-and-implement-splash-screen.md

## Workflow and command law

- agent/commands/README.md
- agent/index/local.main.yaml
- agent/patterns/command-first-execution.md
- agent/patterns/full-replacement-delivery.md
- agent/patterns/index-every-critical-file.md
- agent/patterns/no-guessing-repo-pattern.md
- agent/patterns/pr-driven-delivery.md
- agent/patterns/single-operational-truth.md
- agent/patterns/validation-first-development.md

## Automation and verification law

- .github/pull_request_template.md
- .github/CODEOWNERS
- .github/workflows/ci.yml
- .github/workflows/pr-body.yml
- scripts/verify.sh
- scripts/acp/acp.sh
- scripts/acp/update_progress.py
- scripts/acp/context_tools.py

## Repo-specific preserved overlay truth

- agent/overlay/manifest.json
- agent/overlay/project.yml
- agent/overlay/App/Info.plist
